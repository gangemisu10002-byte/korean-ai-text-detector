...
> **"공개된 KatFish 한국어 데이터셋을 활용하여 Word/Character TF-IDF 기반의 경량 Logistic Regression 모델을 구축하고, Kiwi 형태소·품사 정보를 추가했을 때 AI 생성 텍스트 탐지 성능과 판단이 어떻게 달라지는지를 비교한 프로젝트입니다."**

---

# 22. GitHub에서 파일을 보는 방법

이 문서는 GitHub에서 바로 읽을 수 있는 Markdown 파일이다.
...
GitHub 저장소 최상위에 넣으면 된다.

최종 구조:

```text
korean-ai-text-detector/
├── README.md
├── PROJECT_EXPLANATION.md
├── LICENSE
├── NOTICE
...
├── app.py
└── notebooks/
    └── korean_ai_text_detector_colab_dual_gradio.ipynb
```

`README.md`는 프로젝트의 짧은 소개용이고, `PROJECT_EXPLANATION.md`는 **발표 준비와 코드/실험 설명을 위한 상세 문서**로 사용한다.
